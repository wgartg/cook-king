...
- レシピ投稿(写真・材料・分量・作り方・調理時間・人数分・難易度)
- 一覧表示・キーワード検索・並び替え(新着順/いいね順/調理時間順)
- 自分の投稿は編集・削除が可能。管理者は全レシピを編集・削除可能
- いいね機能・ブックマーク機能(1人1回、ログインしていれば誰でも押せる)
- コメント機能(投稿者が備考・注意点を書き込む用途にも使えます)
- ユーザーごとのページ(投稿者名をクリックすると、そのユーザーの投稿一覧が見られる)
- 共有リンクをコピーすると、Discordなどに貼った時に写真とレシピ名がプレビュー表示される
- 新規投稿されると、設定したDiscordチャンネルに自動でお知らせ(Webhook)が届く
...
export const SHARE_LINK_BASE = "https://your-worker.workers.dev";
```
...
```
recipe-share/
├── index.html            # 一覧・検索・並び替え
├── post.html              # 投稿フォーム(ログイン必須、編集も兼用)
├── detail.html             # 詳細ページ(いいね・ブックマーク・コメント・削除)
├── admin.html              # 管理者ページ
├── user.html               # ユーザーごとの投稿一覧ページ
├── bookmarks.html          # 自分のブックマーク一覧
